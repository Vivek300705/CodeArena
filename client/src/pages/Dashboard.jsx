  ...
  return (
    <div className="container mx-auto px-6 py-12">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        >
          ...
          <div className="space-y-3">
            {loadingStats ? (
              ...
              <p className="text-zinc-600 text-sm py-4 text-center">No submissions yet. <Link to="/problems" className="text-primary">Start solving!</Link></p>
            ) : (
              recent.map((s) => (
                <div key={s._id} className="flex justify-between items-center p-3 bg-background/50 rounded-xl border border-white/5">
                  <div>
                    ...
                    <p className="text-xs text-zinc-500 mt-0.5">{LANG_DISPLAY[s.language] || s.language} • {timeAgo(s.createdAt)}</p>
                  </div>
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${s.verdict === 'Accepted' ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
                    {s.verdict || s.status}
                  </span>
                </div>
              ))
        ...
        >
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold flex items-center gap-2"><Target className="w-5 h-5 text-cyan-400" /> Quick Links</h2>
          ...
          <div className="space-y-3">
            {[
              ...
              <Link key={i} to={to} className="flex items-center gap-3 p-3 bg-background/50 rounded-xl border border-white/5 hover:border-primary/30 hover:bg-primary/5 transition-all group">
